...
fn handle_login_request(client: Client) {
  ...
  // TODO: let #(cipher, plaintext) = cipher.decrypt(client.cipher, bits)
  let #(cipher, plaintext) = #(cipher.nil(), cipher.Plaintext(<<>>))
...
fn deny_login(client: Client, reason: login_denied.Reason) -> Client {
...
fn handle_game_server_selection(
  ...
  Ok(#(client.with_cipher(client, cipher: cipher), game_server))
}
