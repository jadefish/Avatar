...
type ResponseWriter interface {
	// Writer writes the data to the connection as part of an HTTP reply.
	//
...
func (c *conn) readRequest(ctx context.Context) (w *response, err error) {
	...
	w = &response{
		...
		// We populate this ahead of time so we're not
		// reading from req.Header after their Handler starts
		// and maybe mutates it (Issue 14940)
		wantsClose: req.wantsClose(),
	}
	w.w = newBufioWriterSize(w, bufferBeforeChunkingSize)
	return w, nil
}
...
func (c *conn) serve(ctx context.Context) {
	...
	for {
		...
		if err != nil {
			c.server.logf("tcp: error reading request: %v", err)
			return
		}
